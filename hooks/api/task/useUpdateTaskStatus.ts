...
const updateTaskStatus = async (
  ...
  projectId: string,
  taskRef: string,
  request: UpdateTaskStatusFormType
) => {
  const { data } = await axios.put<Task>(
    `/projects/v1/${projectId}/tasks/v1/${taskRef}/status`,
    request,
...
const useUpdateTaskStatus = (projectId: string, taskRef: string) => {
  ...
  return useMutation({
    mutationFn: (request: UpdateTaskStatusFormType) =>
      updateTaskStatus(token!, projectId, taskRef, request),
    onSuccess() {
