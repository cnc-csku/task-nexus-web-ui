...
const updateTaskTitle = async (
  ...
  projectId: string,
  taskRef: string,
  request: UpdateTaskTitleFormType
) => {
  const { data } = await axios.put<Task>(
    `/projects/v1/${projectId}/tasks/v1/${taskRef}/title`,
    request,
...
const useUpdateTaskTitle = (projectId: string, taskRef: string) => {
  ...
  return useMutation({
    mutationFn: (request: UpdateTaskTitleFormType) =>
      updateTaskTitle(token!, projectId, taskRef, request),
    onSuccess() {
