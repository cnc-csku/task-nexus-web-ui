...
const updateProjectAttributesTemplate = async (
  ...
  const { data } = await axios.put<MessageResponse>(
    `/projects/v1/${projectId}/attribute-templates`,
    request,
...
const useUpdateProjectAttributesTemplate = ({ projectId }: Props) => {
  ...
  return useMutation({
    mutationFn: (request: UpdateProjectAttributesTemplatesType) =>
      updateProjectAttributesTemplate(token!, projectId, request),
    onSuccess() {
